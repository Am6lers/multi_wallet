...
export default class Encryptor {
  ...
  _generateKey = (password, salt, lib) =>
    lib === 'original'
      ? Aes.pbkdf2(password, salt, 120000, 256)
  ...
  decryptUseLowIteration = async (password, encryptedString) => {
    const encryptedData = JSON.parse(encryptedString);
    const key = await (encryptedData.lib === 'original'
      ? Aes.pbkdf2(password, encryptedData.salt, 5000, 256)
      : AesForked.pbkdf2(password, encryptedData.salt));
    const data = await this._decryptWithKey(
      encryptedData,
      key,
      encryptedData.lib,
